...
function originAllowed(request: IncomingMessage): boolean {
    ...
    const allowed = new Set([
        ...
        env.discordClientId
            ? `https://${env.discordClientId}.discordsays.com`
            : "",
    ]);
...
export function attachWebSocketServer(
    ...
    const broadcastToSession = (sessionId: string, frame: unknown) => {
        for (const recipient of registry.forSession(sessionId))
            send(recipient.socket, frame);
    };
    ...
    webSockets.on("connection", (socket) => {
        ...
        socket.on("message", async (raw) => {
            try {
                ...
                if (!connection) {
                    ...
                    const context =
                        await application.accessSessions.authenticate(
                            frame.token,
                        );
                    if (!context)
                        throw new AppError(
                    ...
                    send(socket, {
                        ...
                        snapshot: await application.snapshots.get(
                            context.sessionId,
                            context.userId,
                        ),
                    });
                ...
                if (frame.type === "turn.card.endpoint-changed") {
                    await application.rounds.changeEndpoint(
                        connection.context,
                        frame,
                    );
                } else if (frame.type === "presentation.drag.started") {
                    const participantId =
                        await application.rounds.selectPresentationCard(
                            connection.context,
                            frame,
                        );
                    ...
                    const now = Date.now();
                    if (now - drag.lastMovedAt < PRESENTATION_DRAG_INTERVAL_MS)
                        return;
                    ...
                    const drag = activeDrags.get(frame.roundId);
                    if (drag?.userId === connection.context.userId)
                        endDrag(drag);
                    ...
                    if (now - (lastEmoteAt.get(cooldownKey) ?? 0) < 600) {
                        throw new AppError(
                            429,
                            "EMOTE_COOLDOWN",
                            "Wait a moment before emoting again",
                        );
                    }
                    const participantId =
                        await application.rounds.participantForEmote(
                            connection.context,
                            frame.roundId,
                        );
        ...
        socket.on("close", () => {
            ...
            for (const drag of activeDrags.values()) {
                if (
                    drag.sessionId === context.sessionId &&
                    drag.userId === context.userId
                )
                    endDrag(drag);
            }
            if (
    ...
    const unsubscribe = application.events.subscribe(async (sessionId) => {
        await Promise.all(
            registry.forSession(sessionId).map(async ({ socket, context }) => {
                try {
                    const snapshot = await application.snapshots.get(
                        sessionId,
                        context.userId,
                    );
                    const drag = snapshot.round
                        ? activeDrags.get(snapshot.round.id)
                        : undefined;
                    if (
                        drag &&
                        (snapshot.round?.status !== "PLAYING" ||
                            ...
                            snapshot.round.currentPlayerId !==
                                drag.participantId ||
                            snapshot.round.selectedCardId !== drag.cardId ||
                            snapshot.round.placements.some(
                                (placement) => placement.id === drag.cardId,
                            ))
                    )
                        endDrag(drag);
                    send(socket, {
